...
export default async function Page({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-400 via-blue-500 to-indigo-600 py-4 px-3">
      <div className="max-w-sm mx-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-white drop-shadow-lg">Cargo Access Pass</h1>
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200">
          <div className={`px-6 py-4 ${isExpired ? 'bg-red-700' : 'bg-blue-700'}`}>
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className={`text-sm font-bold mt-1 ${isExpired ? 'text-red-100' : 'text-blue-100'}`}>
                  {pass.designation}
                </p>
                <p className={`text-xs font-bold ${isExpired ? 'text-red-200' : 'text-blue-200'}`}>
                  {pass.organization}
                </p>
              </div>
              <div className={`ml-3 px-4 py-2 rounded-xl shadow-lg ${isExpired ? 'bg-red-500/30 border border-red-300' : 'bg-blue-500/30 border border-blue-300'}`}>
          ...
          <div className="p-5">
            ...
            <div className="bg-white rounded-2xl p-4 border-2 border-gray-300 shadow-lg mb-5 text-center">
              <p className={`text-sm font-black uppercase ${isExpired ? 'text-red-600' : 'text-blue-600'}`}>
                CNIC Number
              </p>
            ...
            {pass.areaAllowed?.length > 0 && (
              <div className={`rounded-2xl p-4 border-2 shadow-lg mb-4 ${isExpired ? 'border-red-400' : 'border-blue-400'}`}>
                <h3 className={`text-base font-black mb-3 uppercase ${isExpired ? 'text-red-700' : 'text-blue-700'}`}>Authorized Areas</h3>
                <div className="flex flex-wrap gap-2">
                  {pass.areaAllowed.map((area, idx) => (
                    <span key={idx} className={`px-3 py-2 rounded-full text-sm font-black border-2 shadow ${isExpired ? 'bg-red-50 text-red-800 border-red-400' : 'bg-blue-50 text-blue-800 border-blue-400'}`}>
                      {area}
                    </span>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className={`rounded-2xl p-4 border-2 shadow-lg ${isExpired ? 'border-red-400' : 'border-blue-400'}`}>
                <p className={`text-xs font-black uppercase mb-1 ${isExpired ? 'text-red-700' : 'text-blue-700'}`}>
                  Issue Date
                </p>
          ...
          <div className="bg-gray-800 px-5 py-4 border-t-2 border-gray-300 flex justify-between text-sm">
            <span className={`font-black ${isExpired ? 'text-red-300' : 'text-blue-300'}`}>
              {isExpired ? 'ACCESS DENIED' : 'CARGO AUTHORIZED'}
            </span>
